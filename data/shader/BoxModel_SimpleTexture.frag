...
void main()
{
	...
	pureMaterial.diffuse  = vec3(texture(material.diffuseTex,  TexCoords));
	pureMaterial.specular = vec3(texture(material.specularTex, TexCoords));
	pureMaterial.emission = vec3(texture(material.emissionTex, TexCoords));
	...
	v[1] = v[1] <= 0.01?1:0;
	v[2] = v[2] <= 0.01?1:0;
	result = emissionRatio * v * pureMaterial.emission + (vec3(1,1,1) - v) * result + v * result * (1 - emissionRatio);

	MyCustomOutput = vec4(result,1.0);
}
